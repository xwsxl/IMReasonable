...
-(void)initData
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
       // NSMutableArray  *unsortOneRoomUser=[IMReasonableDao getOneRoomUser:self.from.jidstr];
       // oneRoomUser =[[IMReasonableDao getOneRoomUser:self.from.jidstr] mutableCopy];
        NSSortDescriptor *sort=[[NSSortDescriptor alloc]initWithKey:@"localname" ascending:NO];
        oneRoomUser=[[IMReasonableDao getOneRoomUser:self.from.jidstr] mutableCopy];
        if (oneRoomUser.count<=1) {
        ...
        [oneRoomUser sortUsingDescriptors:[NSArray arrayWithObject:sort]];
        //oneRoomUser=[[array sortedArrayUsingSelector:@selector(compare:)] mutableCopy];

       // oneRoomUser=[unsortOneRoomUser sortedArrayUsingSelector:@selector(compare:)];
