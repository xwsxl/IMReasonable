...
- (NSMutableArray*)chatuserlist
{
    if (_chatuserlist == nil) {
       ...
       // IMChatListModle *model=[[IMChatListModle alloc]init];
      //  model.messagebody.type=EMAIL;
       // [_chatuserlist addObject:model];
      // [_chatuserlist addObjectsFromArray:[IMReasonableDao g]]
    }
...
- (void)tableView:(UITableView*)tableView didSelectRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    if ([tableView cellForRowAtIndexPath:indexPath].tag == MessageTypeEmail) {
       ...
       // temp.messageCount = @"0";
        [self.chatuserlist replaceObjectAtIndex:[indexPath row] withObject:temp];
...
- (void)tableView:(UITableView*)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    if (editingStyle == UITableViewCellEditingStyleDelete) { //如果编辑样式为删除样式
        if (indexPath.row < [self.chatuserlist count]) {

            IMChatListModle* temp = [self.chatuserlist objectAtIndex:[indexPath row]];
            if ([temp.isRoom isEqualToString:@"0"]) {
            [IMReasonableDao clearAllChatMessage:temp.jidstr];
            }
            if ([temp.isRoom isEqualToString:@"1"] && [temp.isimrea isEqualToString:@"1"]) { //是群且不是群成员的时候删除就是清除数据
                ...
                if ([IMReasonableDao deleteUser:temp.jidstr]) { //删除成员并删除消息
                    [self.chatuserlist removeObjectAtIndex:indexPath.row]; //移除数据源的数据
    ...
    if(!self.chatuserlist||self.chatuserlist.count==0){
...
{
    NSLog(@"Chatlist717     message:%@", message);
...
- (void)getfilterData
{
    ...
    if (search.length > 0) {
        ...
        self.chatuserlist = [NSMutableArray arrayWithArray:[self.chatuserlist filteredArrayUsingPredicate:predicate]];
    }
    else {
        self.chatuserlist = [IMReasonableDao getChatlistModle];
        //self.suspendAnimation=true;
...
-(void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath{
   ...
   // NSLog(@"chatlistcount=%ld",self.chatuserlist.count);
