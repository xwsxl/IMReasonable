...
- (void)setMessagemode:(MessageModel *)messagemode isNeedName:(BOOL)isName
{
    ...
    [self.delegate tapMessage:messagemode.content withType:0];
   // _txtContent.backgroundColor=[UIColor grayColor];
    ...
    if (messagemode.isFromMe) {
      ...
      //  NSLog(@"messagemode.comtent=%@",self.messagemode.content);
        ...
        if (isName) {
         ...
         _txtContent.frame=CGRectMake(15, offset, self.frame.size.width-15, self.frame.size.height-offset+2); //设置文本内容
        _username.frame=CGRectMake(15, 0, self.frame.size.width-15, offset-3); //设置显示用户的名字
        _line.frame=CGRectMake(15, offset-3, self.frame.size.width-15, 1);  //设置分割线
