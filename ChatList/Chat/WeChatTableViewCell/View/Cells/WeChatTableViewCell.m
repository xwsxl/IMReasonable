...
-(void)tapGest:(UITapGestureRecognizer *)recognizer
{
   // NSLog(@"xliang");
    [self.delegate loadRequestWithUrl:self.tapUrl withType:self.typexl];
}
-(void)tapMessage:(NSString *)content withType:(TouchContentType)type
{
    self.tapUrl=content;
    self.typexl=type;
}
...
- (void)setMessagemode:(MessageModel*)messagemode isNeedName:(BOOL)isName
{
    ...
    NSLog(@"messagemode.type=%ld",messagemode.type);
    _userPhoto = nil;
    _messageContent = nil;
    _timeAndTips = nil;
    _voiceContent = nil;
   ...
   /* NSLog(@"type=%ld",messagemode.type);
    NSLog(@"content=%@",messagemode.content);
    if(messagemode.type==4)
    {
        if ([messagemode.content containsString:@"/"]) {
            messagemode.type=2;
        }
    }*/
...
- (void)dealwithText
{
    ...
    if (self.messagemode.isFromMe) {
        ...
        if (![_messagemode.isNeedSend isEqualToString:@"1"] && ![_messagemode.isReceived isEqualToString:@"1"]) {
        
        UIActivityIndicatorView *act=[[UIActivityIndicatorView alloc]initWithFrame:CGRectMake(_ScreenWidth - USERPHOTOOFFSET - width - 5-30, USERPHOTOOFFSET / 2, 30, 30)];
...
- (void)dealwithVoice
{
    if (self.messagemode.isFromMe) {
        ...
        if (![_messagemode.isNeedSend isEqualToString:@"1"] && ![_messagemode.isReceived isEqualToString:@"1"]) {
            ...
            UIActivityIndicatorView *act=[[UIActivityIndicatorView alloc]initWithFrame:CGRectMake(_ScreenWidth - USERPHOTOOFFSET - 120 - 5-30, (VOICE_H-40)/2, 30, 30)];
            act.activityIndicatorViewStyle=UIActivityIndicatorViewStyleGray;
           // act.color=[UIColor redColor];
            act.hidesWhenStopped=YES;
            [act startAnimating];
        ...
        _voiceContent.frame = CGRectMake(USERPHOTOOFFSET + 5, (VOICE_H-40)/2, 120, 60);
...
{
    self.messagemode.imageHeight=140;
    self.messagemode.imageWidth=140;
    ...
    if (self.messagemode.isFromMe) {
        ...
        if (tempimg) {
            ...
            if (tempwidth > tempheight) {
              // _messagemode.imageHeight=140*tempheight/tempwidth;
                BOOL flag = tempwidth > (_ScreenWidth - 150) ? true : false;
                _imageWidth = flag ? (_ScreenWidth - 150) : tempwidth;
                _imageHeight = flag ? ((_ScreenWidth - 150) / tempwidth) * tempimg.size.height : tempimg.size.height;
                
            }
            else {
                //_messagemode.imageWidth=140*tempwidth/tempheight;
                float zh = tempimg.size.width / tempimg.size.height;
                _imageWidth = 140 * zh > (_ScreenWidth - 150) ? 140.0 : 140 * zh;
            }
            _pictureContent.frame = CGRectMake(_ScreenWidth-_messagemode.imageWidth-MARGEN, USERPHOTOOFFSET / 2, _messagemode.imageWidth,_messagemode.imageHeight);
        ...
        width += 15;
        _messagemode.imageHeight=IMAGE_HEIGHT+offset+20;
  ...
  //  NSLog(@"self.messagemode.imageHeight=%f",self.messagemode.imageHeight);
   // NSLog(@"content=%@",self.messagemode.content);
}
...
+(CGFloat)getCellHeight:(MessageModel*)messagemode isNeedName:(BOOL)isName
{
    ...
    case MessageTypePicture: { //图片消息
        return [WeChatTableViewCell getPictureHeight:messagemode isNeedName:(BOOL)isName];
    // return _imageHeight;
    } break;
    case MessageTypeTips: { //消息提示
        cellheight = 20;
    ...
    case MessageTypeVoice: { //声音消息
        ...
        if (!isName) {
            cellheight=VOICE_H_M;
        }
      //  NSLog(@"url=%@,username=%@",messagemode.faceurl,messagemode.username);
    } break;
    case MessageTypeText: { //文本消息
...
+ (CGFloat)getPictureHeight:(MessageModel*)messagemode isNeedName:(BOOL)isName
{
   // NSLog(@"messagemode.imageHeight=%f",messagemode.imageHeight);
    if (isName) {
        return messagemode.isFromMe?140+20:180+30;
    }
    return messagemode.isFromMe?140+20:170+20;
}
