...
@interface ChatViewController()<PJsendImageDelegate,UIWebViewDelegate>
...
@property(nonatomic,strong)UIWebView *chatWeb;
@property(nonatomic,strong)SpreadMailModel *model;
@property(nonatomic,assign)BOOL DeleteMsg;
//@property(nonatomic,assign) CGFloat tableviewHeight;
@property(nonatomic,strong)NSMutableArray *resendMsgList;
@property(nonatomic,strong)NSMutableArray *resendImg;
@property(nonatomic,strong)NSTimer *timer;
@property(nonatomic,assign)BOOL hasStartTimer;
@property(nonatomic,assign)BOOL hasStartTimer2;
...
-(NSMutableArray *)resendImg
{
    if (!_resendImg) {
        _resendImg=[[NSMutableArray alloc]init];
    }
    return _resendImg;
}
-(NSMutableArray *)resendMsgList
{
    if (!_resendMsgList) {
        _resendMsgList=[[NSMutableArray alloc]init];
    }
    return _resendMsgList;
}
-(NSTimer *)timer
{
    if (!_timer) {
        _timer=[NSTimer timerWithTimeInterval:3.0 target:self selector:@selector(timerFired) userInfo:nil repeats:YES];
...
{
    if (!_timer2) {
        _timer2=[NSTimer timerWithTimeInterval:3.0 target:self selector:@selector(resendImg1) userInfo:nil repeats:YES];
        [[NSRunLoop  currentRunLoop] addTimer:_timer2 forMode:NSDefaultRunLoopMode];
    }
    return _timer2;
}
-(SpreadMailModel *)model
{
    if (!_model) {
        _model=[[SpreadMailModel alloc]init];
    }
    return _model;
}
...
- (void)initViewControl
{
    ...
    //    tableview.backgroundView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:chatwallpaper]];//hahahaha
...
- (void)hidenKeyboard
{
    ...
    NSString* fromjidstr = self.from.jidstr;
    [IMReasonableDao setTempText:fromjidstr text:[key getText]];
    [key hideKeyboard];
    [self initData:YES];
}
...
- (void)initNav
{
   ...
   // NSString* title = [NSString stringWithFormat:NSLocalizedString(@"lbtalkwith", nil), temptitle]; //lbtalkwith
    if ([UIScreen mainScreen].applicationFrame.size.height<[UIScreen mainScreen].applicationFrame.size.width) {
        titleView=[[TitleView alloc]initWithFrame:CGRectMake(0, 0, 200, 38)];
    }else{
    titleView = [[TitleView alloc] initWithFrame:CGRectMake(0, 0, 200, 44)];
    }
    titleView.title.text = temptitle;
    ...
    if ([XMPPDao sharedXMPPManager].isConnectInternet) { //判断是否有网络连接
        ...
        if ([self.from.isRoom isEqualToString:@"0"] && [self.from.state isEqualToString:@"available"]) { //判断是否连接到聊天服务器
            ...
            //    NSLog(@"%@",self.from.update);
                if ([self.from.isRoom isEqualToString:@"0"]) {
                    if (self.from.update.length>10) {
...
- (void)initNotinGroupMsg
{
    ...
    //    "lbcvcGroupchat"="";
    notingroup.text = NSLocalizedString(@"lbcvcGroupchat", nil); //@"您已经不是该群成员，不能在该群发消息";
...
{
     double offset1=-64;
    if(tableview.contentSize.height>self.view.frame.size.height-64)
    {
        offset1=-(self.view.frame.size.height - (tableview.frame.origin.y + tableview.contentSize.height + (self.view.frame.size.height - key.frame.origin.y)));
       // offset1=tableview.contentSize.height-self.view.frame.size.height+keyboardHight;
    }
    
    [tableview setContentOffset:CGPointMake(0, offset1) animated:NO];
    //[tableview setContentOffset:CGPointMake(0, offset1) ];
...
- (void)goLastMessage
{
    ...
    if (offset < 78) {
        ...
        [tableview setContentOffset:CGPointMake(0, -offset) animated:NO];
    }
    else {
...
{
    NSLog(@"messagelist=%ld",messageList.count);
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
   ...
   // NSLog(@"time=%@",mode.time);
        return cell;
}
- (CGFloat)tableView:(UITableView*)tableView heightForRowAtIndexPath:(NSIndexPath*)indexPath
{
    //WeChatTableViewCell *cell=[[WeChatTableViewCell alloc]init];
    CGFloat height = [WeChatTableViewCell getCellHeight:[messageList objectAtIndex:[indexPath row]] isNeedName:isRoom];
    NSLog(@"IndexPath=%ld,Height=%2f",indexPath.row,height);
    return height;
}
...
- (void)choiceFuction:(NSUInteger)functionid
{
    ...
    if ([XMPPDao sharedXMPPManager].isConnectInternet||1) {
        switch (functionid) {
        case 900:
            //相册
...
- (void)sendMessage:(NSString*)body type:(NSString*)type voicelenth:(NSString*)lenth voicepath:(NSString*)path
{
    ...
    if (![str isEqualToString:@""]) {
        ...
        if ([type isEqualToString:@"img"]) {
            [IMReasonableDao updateMsgID:oneimgmessage.ID whitMsgID:msgID];
            oneimgmessage.ID=msgID;
        }
    ...
    if ([XMPPDao sharedXMPPManager].isConnectInternet&&[XMPPDao sharedXMPPManager].isLogin&&![type isEqualToString:@"voice"]) {
        ...
        else
        {
            ...
            model.username=self.from.jidstr;
            model.faceurl=type;
            model.content=body;
            model.time=lenth;
            model.ID=msgID;
            [self.resendMsgList addObject:model];
            // [IMReasonableDao saveMsgWithJidstr:self.from.jidstr type:type body:str vl:lenth msgID:msgID];
            self.timer=[NSTimer timerWithTimeInterval:3.0 target:self selector:@selector(timerFired) userInfo:nil repeats:YES];
            [[NSRunLoop  currentRunLoop] addTimer:self.timer forMode:NSDefaultRunLoopMode];
            if (!self.hasStartTimer) {
                [self.timer fire];
                self.hasStartTimer=YES;
            }
...
-(void)timerFired
{
    NSArray *array=[self.resendMsgList copy];
    if ([XMPPDao sharedXMPPManager].isConnectInternet&&[XMPPDao sharedXMPPManager].isLogin) {
        for (int i=0; i<array.count; i++) {
            MessageModel * model1=[array objectAtIndex:i];
            
            NSString *msgID=[Tool GetOnlyString];
            [self SendMessage:model1.faceurl body:model1.content voicelenth:model1.time msgID:msgID];
            [IMReasonableDao updateMsgID:model1.ID whitMsgID:msgID];
            [IMReasonableDao setUser:self.from.jidstr msg:msgID];
            [NSThread sleepForTimeInterval:0.8f];
            
        }
        
        [self.resendMsgList removeAllObjects];
        if(_timer){
...
-(void)didSendImage:(UIImage *)image AndIsOriginal:(BOOL)isOriginal{
    ...
    else
    {
       ...
       NSDictionary *dic=[[NSDictionary alloc]initWithObjects:@[url,tempJsonData,tempmeseage] forKeys:@[@"url",@"data",@"msg"]];
        [self.resendImg addObject:dic];
        if (!self.hasStartTimer2) {
            self.timer2=[NSTimer timerWithTimeInterval:3.0 target:self selector:@selector(resendImg1) userInfo:nil repeats:YES];
            [[NSRunLoop  currentRunLoop] addTimer:_timer2 forMode:NSDefaultRunLoopMode];
        [self.timer2 fire];
        self.hasStartTimer2=YES;
        }
    }
...
{
    if ([XMPPDao sharedXMPPManager].isConnectInternet&&[XMPPDao sharedXMPPManager].isLogin) {
        NSArray *array=[_resendImg copy];
        for (int i=0; i<array.count; i++) {
            NSDictionary *dic=[array objectAtIndex:i];
            NSURL *url=[dic objectForKey:@"url"];
            NSMutableData *tempJsonData=[dic objectForKey:@"data"];
            MessageModel *tempmeseage=[dic objectForKey:@"msg"];
            ASIHTTPRequest* request = [ASIHTTPRequest requestWithURL:url];
            [request addRequestHeader:@"Content-Type" value:@"application/json; encoding=utf-8"];
            [request addRequestHeader:@"Accept" value:@"application/json"];
            [request setRequestMethod:@"POST"];
        ...
        self.timer2=nil;
            self.hasStartTimer2=NO;
        }
        [_resendImg removeAllObjects];
    }
}
...
{
   // NSLog(@"哈哈哈哈哈哈哈哈哈message:%@",message);
    ...
    [IMReasonableDao setUserUpdate:message.date user:message.from];
...
-(void)loadRequestWithUrl:(NSString *)url withType:(TouchContentType)type
{
    //TQRichTextRunURL *str=url;
    
    
    [key hideKeyboard];
    NSMutableAttributedString *str=[[NSMutableAttributedString alloc]initWithString:url];
    NSArray *array=[TQRichTextRunURL runsForAttributedString:str];
    if (array.count>0) {
        TQRichTextRunURL *str2=array.lastObject;
        if ([str2.text rangeOfString:@"://"].location==NSNotFound) {
            str2.text=[@"http://" stringByAppendingString:str2.text];
        }
        EmailDetailViewController *emailDetailViewController=[[EmailDetailViewController alloc] init];
        self.model.newsletterLinkUrl=str2.text;
        emailDetailViewController.hidesBottomBarWhenPushed=YES;
        emailDetailViewController.model=self.model;
