...
- (void)viewDidLoad
{
    ...
    self.emailLab.text=NSLocalizedString(@"Email", nil);
...
- (void)GetContacts
{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        for (int i = 0; i < CFArrayGetCount(results); i++) {
            ...
            for (int k = 0; k < ABMultiValueGetCount(phone)&&k<10; k++) {
                ...
                if (![tmpPhoneIndex isEqualToString:@""]&&(tmpPhoneIndex.length>=7)) {
                   ...
                   else if (![unknowphone isEqualToString:@"+"]) { //需要当前用户的国家代码
            ...
            dispatch_async(dispatch_get_global_queue(0, 0), ^{
                [RespreadSoapTool Soap:WSDL_URL
                    ...
                    success:^(id success) {
                        ...
                        //保存用户填写的邮箱
                        [defaults setObject:inputemail forKey:MyEmail];
