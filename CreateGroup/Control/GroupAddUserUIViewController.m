...
- (void)CreatGroup:(UIBarButtonItem*)btn
{
    ...
    NSString *roomjid=[[XMPPRoomDao sharedXMPPManager] createRoom:roomname];
    
    [self sendMessage:NSLocalizedString(@"lbngvstartchat", nil) type:@"chat" voicelenth:@"0" voicepath:@"" roomName:roomjid];
}
- (void)sendMessage:(NSString*)body type:(NSString*)type voicelenth:(NSString*)lenth voicepath:(NSString*)path roomName:(NSString *)roomname
{
    ...
    NSString *myJID = [[NSUserDefaults standardUserDefaults] stringForKey:XMPPREASONABLEJID];
        ...
        [[XMPPDao sharedXMPPManager] sendChatRoomMessage:roomname type:type body:body voicelenth:lenth msgID:msgID];
    //[IMReasonableDao SaveGroupMessage2:msgID fromjidstr:self.from.jidstr tojidstr:myJID body:body type:type date:time groupfrom:roomname voicelenth:lenth];
    
   [IMReasonableDao saveMessage2:roomname to:myJID body:body type:type date:time voicelenth:lenth msgID:msgID]; //存入本地数据
    [IMReasonableDao setUser:roomname msg:msgID];
    NSArray *array=[IMReasonableDao getChatlistModle];
