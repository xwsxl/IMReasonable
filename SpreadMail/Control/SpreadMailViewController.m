...
-(NSMutableArray *)emailArray{
    if(_emailArray==nil){
        ...
        NSLog(@"emailcount=%d",amount);
        
        _emailArray=[IMReasonableDao getEmailArray:JID WithPagerNumber:self.pagerNumber+6 AndCount:EMAIL_COUNT];
...
-(long)pagerNumber{
    if(_pagerNumber<=0){
        ...
        self.totalCount=[IMReasonableDao getEmailCount];
        
        _pagerNumber=[self getNextPagerNumber:self.totalCount EmailCount:EMAIL_COUNT];
...
- (void)initNavTitle{
  ...
  /*  UIBarButtonItem *backbutton=[[UIBarButtonItem alloc]initWithTitle:NSLocalizedString(@"BACK", nil) style:UIBarButtonItemStylePlain target:self action:@selector(goBack)];
...
-(void)LoadData{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSArray *tempArray;
        NSLog(@"self.pagernumber=%ld",_pagerNumber);
        if(_pagerNumber>=0){
...
- (void)setMessageReaded
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSMutableArray *array=[IMReasonableDao getEmailArray:JID WithPagerNumber:self.pagerNumber AndCount:EMAIL_COUNT];
        [array addObjectsFromArray:self.emailArray];
