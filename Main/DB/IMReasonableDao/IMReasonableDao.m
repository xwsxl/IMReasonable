           ...
           voicelenth:(NSString *)voicelenth msgID:(NSString *) msgID
{
   // NSLog(@"body:%@",body);
    ...
    NSString *body1=[[NSString alloc]init];
    if ([type isEqualToString:EMAIL]) {
       NSString *  body2=[body stringByReplacingOccurrencesOfString:@"'" withString:@"%quert%"];
        body1=[body2 stringByReplacingOccurrencesOfString:@"\"" withString:@"'"];
        
    }else
    {
    body1=[body stringByReplacingOccurrencesOfString:@"\"" withString:@"\"\""];
    }
    NSString * sql=[NSString stringWithFormat:@"insert into Message([msgID],[from],[to],[body],[type],[date],[isneedsend],[isaccpet],[voicelenth]) values(\"%@\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\")",msgID,from,to,body1,type,date,@"1",@"1",voicelenth];
        BOOL result=[FMDBDao executeUpdate:sql];
    @try {
        [FMDBDao executeQuery:sql];
    } @catch (NSException *exception) {
        NSLog(@"异常原因：%@，异常名称：%@",[exception name],[exception reason]);
    } @finally {
        
    }
    for (; !result;) {
        
       result=[FMDBDao executeUpdate:sql];
    }
    NSLog(@"sql1:%@",sql);
    NSLog(@"result:%d",result);
    NSString * sql2=[NSString stringWithFormat:@"insert into [User](jidstr,[msgID],[isactive],[isloc],[unreadcount]) values(\"%@\",\"%@\",\"%@\",\"%@\",\"%@\")",from,msgID,@"1",@"1",@"1"];
    NSLog(@"sql2:%@",sql2);
        BOOL flag= [FMDBDao executeUpdate:sql2];
        if (!flag) {
             sql2=[NSString stringWithFormat:@"update [User] set msgID=\"%@\" ,[isloc]=\"1\"  where jidstr= \"%@\"  ",msgID,from];
            [FMDBDao executeUpdate:sql];
        }
    sql=[NSString stringWithFormat:@"update [User] set [update]=\"%@\"  where jidstr= \"%@\" ",date,from] ;
    [FMDBDao executeUpdate:sql2];
       return flag;
}
...
+(void)setUserUpdate:(NSString *)update user:(NSString *)jidstr
{
    NSString *sql;
    sql=[NSString stringWithFormat:@"update [user] set [update]=\"%@\" where jidstr=\"%@\"",update,jidstr];
    [FMDBDao executeUpdate:sql];
}
+(void)setUser:(NSString *)jidstr msg:(NSString *)msgID
{
    
    NSString *sql=[NSString stringWithFormat:@"insert into [User]([jidstr],[msgID]) values(\"%@\",\"%@\")",jidstr,msgID];
    [FMDBDao executeQuery:sql];
}
...
+(NSMutableArray*)getChatlistModle
{
    ...
    BOOL flag=false;
    NSString *sql2=@"select jidstr from [user] order by [update] desc";
    FMResultSet *rs2=[FMDBDao executeQuery:sql2];
    [rs2 next];
    NSString *jidstr=[rs2 stringForColumn:@"jidstr"];
    [rs2 close];
    sql2=[NSString stringWithFormat:@"SELECT msgID FROM Message  where [from]=\"%@\" or [to]=\"%@\" order by  date desc limit 0,1",jidstr,jidstr];
    FMResultSet *rs3=[FMDBDao executeQuery:sql2];
    [rs3 next];
    NSString *msgID=[rs3 stringForColumn:@"msgID"];
    [rs3 close];
    sql2=[NSString stringWithFormat:@"update [User] set msgID=%@ where jidstr= \"%@\" ",msgID,jidstr];
    [FMDBDao executeUpdate:sql2];
    ...
    while ([rs next]){
        ...
        if (tempuser.update) {
            ...
            NSDate* date=[formatter dateFromString:tempuser.update];
            NSTimeInterval time=8*60;
            NSDate *date2=[date dateByAddingTimeInterval:time];
            tempuser.update=[formatter stringFromDate:date2];
            
            if ([self string1:[formatter stringFromDate:[NSDate dateWithTimeIntervalSinceNow:0]] compareTo:tempuser.update]) {
                //tempuser.update=@"在线";
                tempuser.update=NSLocalizedString(@"lbonline", nil);
            }
...
+(BOOL)string1:(NSString*) string1 compareTo:(NSString *)string2
{
   ...
   // NSString * sql=@"select * from User where isimrea=\"1\"  or isloc=\"1\" order by [update] desc,[device] desc";
    ...
    while ([rs next]){
      ...
      //  NSString *jidstr=[rs stringForColumn:@"jidstr"];
       ...
       // tempuser.localname=tempuser.localname?tempuser.localname:tempuser.nick;
        tempuser.phonenumber=[rs stringForColumn:@"phonenumber"];
        tempuser.faceurl=[rs stringForColumn:@"faceurl"];
        tempuser.group=[rs stringForColumn:@"group"];
        tempuser.update=[rs stringForColumn:@"update"];
        ...
        if (tempuser.phoneown==NULL) {
            [chatuserlist addObject:tempuser];
        }
        else if ([tempuser.phoneown rangeOfString:@"詐騙電話"].location==NSNotFound&&
            [tempuser.phoneown rangeOfString:@"騷擾號碼"].location==NSNotFound&&
            [tempuser.phoneown rangeOfString:@"廣告推銷"].location==NSNotFound&&
            [tempuser.phoneown rangeOfString:@"電訊廣播"].location==NSNotFound&&
            [tempuser.phoneown rangeOfString:@"財務借貸"].location==NSNotFound&&
            [tempuser.phoneown rangeOfString:@"銀行投資"].location==NSNotFound&&
            [tempuser.phoneown rangeOfString:@"美容瘦身"].location==NSNotFound&&
            [tempuser.phoneown rangeOfString:@"會籍消費"].location==NSNotFound&&
            [tempuser.phoneown rangeOfString:@"保險"].location==NSNotFound&&
            [tempuser.phoneown rangeOfString:@"教育"].location==NSNotFound&&
            [tempuser.phoneown rangeOfString:@"诈骗"].location==NSNotFound&&
            [tempuser.phoneown rangeOfString:@"广告推销"].location==NSNotFound&&
            [tempuser.phoneown rangeOfString:@"骚扰号码"].location==NSNotFound) {
        [chatuserlist addObject:tempuser];
        }
        [tempuser autorelease];
    }
...
{
   // @"select * from (      select * from (        select a.*,b.localname,b.faceurl from (  select * from Message where  ([from] =\"%@\" and [to] =\"%@\") or ([from] =\"%@\" and [to] =\"%@\") )  a left join [user] b on a.[from]=b.jidstr    order by [date] desc ) as ms  limit 0,%@  )  order by [date]"
    NSMutableArray* messagelist=[[NSMutableArray alloc] init];
  //  NSString * sql=[NSString stringWithFormat:@"select * from (      select * from (        select a.*,b.localname,b.faceurl from (  select * from Message where  ([from] =\"%@\" and [to] =\"%@\") or ([from] =\"%@\" and [to] =\"%@\") )  a left join [user] b on a.[from]=b.jidstr    order by [date] desc )   )  order by [date]",fromjidstr ,tojidstr,tojidstr,fromjidstr];
        ...
        if([type isEqualToString:EMAIL]){
           ...
           // NSLog(@"%@",[rs stringForColumn:@"body"]);
            ...
            temp.type=MessageTypeEmail;
             ...
             temp.content=[rs stringForColumn:@"body"];
          //  NSLog(@"temp.content=%@",temp.content);
...
+ (void) saveMessage2:(NSString *) from to:(NSString *) to body:(NSString *) body type:(NSString *) type date:(NSString *)date voicelenth:(NSString *)lenth msgID:(NSString *) msgID
{
    ...
    NSString * sql=[NSString stringWithFormat:@"insert into Message([msgID],[from],[to],[body],[type],[date],[isneedsend],[isaccpet],[voicelenth]) values(\"%@\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\",\"%@\")",msgID,from,to,body1,type,date,@"2",@"0",lenth];
        [FMDBDao executeUpdate:sql];
  //  NSLog(@"body=%@",body);
   // NSLog(@"sql5=%@",sql);
...
+(NSMutableArray*)getChatRoomMessage2:(NSString *)roomjidstr rowCount:(NSString *)rowcount
{
    ...
    while ([rs next]){
        ...
        temp.ID=[rs stringForColumn:@"msgID"];
        NSString * tojidstr=[rs stringForColumn:@"to"];
        NSString *  name=[rs stringForColumn:@"groupfrom"];
        temp.isFromMe=false;
        if (([tojidstr rangeOfString:myjidstr].location ==NSNotFound)||([name rangeOfString:myjidstr].location!=NSNotFound)) {
            temp.isFromMe=true;
        }
        NSString * data=[rs stringForColumn:@"date"];
        temp.time=[Tool getHHMM:data];  
...
+(NSMutableArray *)getAllUser
{
    ...
    NSString * sql=@"select jidstr , phonenumber,localname,phonetitle from [User] where isloc !=\"1\" and isRoom !=\"1\"";
    
    FMResultSet *rs=[FMDBDao executeQuery:sql];
    while ([rs next]){
        NSString *phonetitle=[rs stringForColumn:@"phonetitle"];
        NSString * nickname=[rs stringForColumn:@"localname"];
        if ([nickname rangeOfString:@"JunkCall"].location==NSNotFound) {
        if ([phonetitle rangeOfString:@"詐騙電話"].location==NSNotFound&&
            [phonetitle rangeOfString:@"騷擾號碼"].location==NSNotFound&&
            [phonetitle rangeOfString:@"廣告推銷"].location==NSNotFound&&
            [phonetitle rangeOfString:@"電訊廣播"].location==NSNotFound&&
            [phonetitle rangeOfString:@"財務借貸"].location==NSNotFound&&
            [phonetitle rangeOfString:@"銀行投資"].location==NSNotFound&&
            [phonetitle rangeOfString:@"美容瘦身"].location==NSNotFound&&
            [phonetitle rangeOfString:@"會籍消費"].location==NSNotFound&&
            [phonetitle rangeOfString:@"保險"].location==NSNotFound&&
            [phonetitle rangeOfString:@"教育"].location==NSNotFound&&
            [phonetitle rangeOfString:@"诈骗"].location==NSNotFound&&
            [phonetitle rangeOfString:@"詐騙"].location==NSNotFound&&
            [phonetitle rangeOfString:@"广告推销"].location==NSNotFound&&
            [phonetitle rangeOfString:@"骚扰号码"].location==NSNotFound) {
...
+(void)updateMsgID:(NSString *)msgID1 whitMsgID:(NSString *)msgID2
{
    NSString *sql=[NSString stringWithFormat:@"update [message] set [msgID]=\"%@\" where [msgID]=\"%@\"",msgID2,msgID1];
    [FMDBDao executeUpdate:sql];
}
//删除联系人
...
+(void)deleteUser
{
    NSString *sql=[NSString stringWithFormat:@"delete from [user] where addrefID is not null and isactive=\"0\""];
    [FMDBDao executeUpdate:sql];
    //NSString *sql=[NSString stringWithFormat:@""];
}

//添加好友
+(void)saveUser:(NSString *)jidstr image:(NSString *)nick
{
    NSString * sql=[NSString stringWithFormat:@"insert into User(jidstr,localname) values(\"%@\",\"%@\")",jidstr,nick];//phonetitle
    
    
    BOOL flag=[FMDBDao executeUpdate:sql];
    if (!flag) {
        sql=[NSString stringWithFormat:@"update User set localname =\"%@\" ,   WHERE jidstr = \"%@\" ",nick,jidstr];
